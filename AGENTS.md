...
- **在 worktree 里起开发服务器（`bun run dev` → `scripts/dev-supervisor.sh`，gateway `--watch` + vite）**：dev 走 `development.env`，其 `DATABASE_URL=./tmex.db` 被 `loadEnv()` 解析到**当前仓库根**——在 worktree 里即 worktree 根（一个空库），不会自动用主仓的 dev 数据。若要带主仓已有 dev 数据调试，**先从主仓文件系统复制 dev 库**（WAL 模式，三件套一起拷）：`cp <主仓根>/tmex.db{,-shm,-wal} <worktree根>/`（主仓根即原始 clone 路径，如 `/Users/krhougs/LocalCodes/tmex`；复制前确认无 dev 进程占用主库）。**严禁从生产环境取数**——生产库在 `~/Library/Application Support/tmex/data/tmex.db`（常驻服务，监听 9883），绝不复制/读取/覆盖。拷完直接 `bun run dev`，迁移在启动时自动应用（新 schema 会补到拷来的库上）。dev 端口 19663/19883（避开生产 9663/9883）；若主仓 dev 同时在跑，用 `development.env.local`（已 gitignore）覆盖 `GATEWAY_PORT`/`FE_PORT`/`TMEX_BASE_URL`/`TMEX_GATEWAY_URL` 避免端口冲突。
