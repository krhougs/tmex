...
- **严禁触碰名为 `tmex` 的 tmux session**：开发与测试过程中，执行任何 tmux 操作（attach、send-keys、kill-session、kill-server、capture-pane、resize 等）前，永远需要仔细检查目标是不是名为 `tmex` 的 session——它属于生产/日常工作环境，严禁读写、注入输入或杀掉。测试一律使用独立 tmux socket（如 `tmux -L tmex-e2e`）或专用测试 session 名；尤其注意 `kill-server` 作用于整个 socket，绝不允许在默认 socket 上执行。
- **三套环境（`NODE_ENV` = development / test / production）**：开发服务器走 `development`（仓库根 `development.env`），所有测试走 `test`（仓库根 `test.env`），`production` 仅限打包安装的常驻服务（变量来自安装版 `app.env` + `run.sh`，绝不读仓库 env 文件）。Rust Gateway 入口自行加载对应 env；共享加载器 `packages/shared/src/env/load-env.ts` 仅供保留的 TypeScript oracle/preload 使用。它是 Node-only，**不要从 `@tmex/shared` 浏览器侧主入口导出**（否则进前端 bundle 报 `node:fs externalized`），Node 侧一律相对 import。前端（vite）只读 `TMEX_GATEWAY_URL`/`FE_PORT` 两个非密钥接线值，**不加载后端 env**。个人临时覆盖写 `*.env.local`（已 gitignore）。详见 `docs/env/2026061301-three-tier-env.md`。
- **在 worktree 里起开发服务器（`bun run dev` → `scripts/dev-supervisor.sh`，Rust Gateway + Vite）**：dev 走 `development.env`，其 `DATABASE_URL=./tmex.db` 被 Rust Gateway 入口解析到**当前仓库根**——在 worktree 里即 worktree 根（一个空库），不会自动用主仓的 dev 数据。若要带主仓已有 dev 数据调试，**先从主仓文件系统复制 dev 库**（WAL 模式，三件套一起拷）：`cp <主仓根>/tmex.db{,-shm,-wal} <worktree根>/`（主仓根即原始 clone 路径，如 `/Users/krhougs/LocalCodes/tmex`；复制前确认无 dev 进程占用主库）。**严禁从生产环境取数**——生产库在 `~/Library/Application Support/tmex/data/tmex.db`（常驻服务，监听 9883），绝不复制/读取/覆盖。拷完直接 `bun run dev`，编译进 binary 的 migration 会在启动时自动应用。dev 端口 19663/19883（避开生产 9663/9883）；若主仓 dev 同时在跑，用 `development.env.local`（已 gitignore）覆盖 `GATEWAY_PORT`/`FE_PORT`/`TMEX_BASE_URL`/`TMEX_GATEWAY_URL` 避免端口冲突。
