...
test.describe.serial('agent session', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    providerId = await createProviderAndSetDefault(request, mock.baseUrl, `e2e-mock-${Date.now()}`);
  ...
  test('provider unreachable shows error banner with retry', async ({ page, request }) => {
    ...
    const settingsRes = await request.patch('/api/llm/settings', {
      data: { defaultProviderId: deadProvider.provider.id, defaultModelId: 'mock-model' },
    });
    expect(settingsRes.ok()).toBeTruthy();
