...
import type { Device, CreateDeviceRequest } from '@tmex/shared';
...
function DeviceCard({ device, onDelete }: DeviceCardProps) {
  ...
  const subtitle = device.type === 'local'
    ? '本地设备'
      ...
      <div className="bg-bg-secondary rounded-lg border border-border w-full max-w-lg max-h-[90vh] overflow-auto">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            ...
            <select
              value={formData.type}
              onChange={(e) => setFormData(d => ({ ...d, type: e.target.value as 'local' | 'ssh' }))}
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.port}
                    onChange={(e) => setFormData(d => ({ ...d, port: parseInt(e.target.value) }))}
                    className="input w-full"
                    min={1}
                    max={65535}
                  />
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData(d => ({ ...d, authMode: e.target.value as CreateDeviceRequest['authMode'] }))}
              className="select w-full"
            >
          ...
          {formData.authMode === 'key' && (
            <>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">私钥密码（可选）</label>
                <input
                  type="password"
                  value={formData.privateKeyPassphrase}
                  onChange={(e) => setFormData(d => ({ ...d, privateKeyPassphrase: e.target.value }))}
                  className="input w-full"
                />
              </div>
            </>
