...
## 终端尺寸新回归 Prompt（2026-04-15）

> 现在修引入的新bug：
> 1. 进入页面/页面刷新/切换pane均不能正确设置终端宽高
> 2. resize窗口不能正确设置终端宽高，但是resize之后如果在外部观察tmux窗口（如iterm2的tmux集成），能看到对应pane闪了一下

## 终端尺寸补充现象 Prompt（2026-04-15）

> 依然复现，我帮你做了简单的debug：
> 1. xterm区域没有完全撑满其wrapper
> 2. 只有resize到比当前终端要小的宽度/高度时才resize才生效
> 3. 无论怎么resize高度都不对，只有切换窗口后终端区域高度才会被重新设置
