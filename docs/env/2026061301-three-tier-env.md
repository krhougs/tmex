...
tmex 此前只有两个事实上的 `NODE_ENV` 取值（`development` / `production`），环境变量加载逻辑散落在 `dev-supervisor.sh`、两个 `test-preload.ts`、`playwright.config.ts` 等多处并互相打补丁；dev / test 运行还会继承 shell 里安装版 `app.env` 的变量（`NODE_ENV=production`、生产 `DATABASE_URL`、指向安装目录的 `TMEX_FE_DIST_DIR`），多次导致 dev 启动崩溃、单元测试误写生产库。
...
| production | `production` | 安装版 `app.env`（`buildAppEnvValues`）+ `run.sh` export 的 `TMEX_FE_DIST_DIR` | `run.sh` 经 shell 注入 |
...
- fail-fast 校验生产契约：`TMEX_MASTER_KEY`/`GATEWAY_PORT`/`TMEX_BIND_HOST`/`DATABASE_URL` 必须存在，`TMEX_FE_DIST_DIR` 必须存在且指向真实目录（`existsSync`）。任一缺失即抛带可操作信息的错误（提示检查 app.env / 重跑 `tmex upgrade`）。Rust Gateway 的 migration 已编译进 binary，不读取 migration 路径。
- 打印生产摘要作为可观测信号。

**development / test 分支 `applyRepoEnv()`**

1. 净化继承的安装版 `TMEX_FE_DIST_DIR`：若它含 `Application Support/tmex` 标记则删除（收敛旧 hack）。
...
| 入口 | 接入方式 |
|---|---|
| dev gateway | `scripts/dev-supervisor.sh` 启动 `apps/gateway/scripts/run-rust-gateway.sh`；Rust binary 自行加载 repository env |
| 生产 runtime | `tmex-cli` 的 `run.sh` 直接 exec 当前平台 `tmex-gateway` artifact |
...
- **生产路径键保护**：`applyProductionEnv()` 的早返回必须在净化逻辑之前——否则会删掉生产正确的 `TMEX_FE_DIST_DIR`，搞崩常驻服务。单测已钉死该行为。
