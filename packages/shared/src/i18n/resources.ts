...
export const I18N_RESOURCES = {
  ...
  "translation": {
    ...
    "nav": {
      ...
      "forceTerminalResize": "Force terminal resize",
    ...
    "terminal": {
      ...
      "pasteTooLarge": "Paste is too large ({{size}} MiB); the limit is {{limit}} MiB",
      "imagePasteTooLarge": "Image is too large ({{size}}); the limit is {{limit}}",
      "filePasteTooLarge": "File is too large ({{size}}); the limit is {{limit}}",
      "filePasteNoRoot": "Temporary transfer directory unavailable; cannot transfer files",
      "filePasteUploaded": "File transferred: {{path}}",
      "filePastePathCopied": "{{count}} file path(s) copied because direct terminal input is unavailable",
      "filePastePathCopyFailed": "Files were transferred, but their paths could not be inserted or copied",
      "filePasteFailed": "File transfer failed: {{message}}",
      "remoteDirectoryPasteUnsupported": "Remote terminals only accept files; skipped {{count}} folder(s)",
      "pathPasteUnavailable": "{{count}} selected path(s) could not be accessed",
      "fileDropLocalTitle": "Release to paste local paths",
      "fileDropUploadTitle": "Release to transfer files to this terminal",
      "fileDropHint": "{{count}} file(s) · paths are inserted without running them",
      "fileRiskTitle": "Review files before adding them",
      "fileRiskDescription": "Some files are not common images or text. Adding a path does not run a file, but later terminal commands may read or execute it. Continue only if you trust the source.",
      "fileRiskMore": "And {{count}} more",
      "fileRiskBatch": "{{count}} files will be processed after confirmation",
      "fileRiskConfirm": "Continue",
  ...
  "translation": {
    ...
    "terminal": {
      ...
      "pasteTooLarge": "粘贴内容过大（{{size}} MiB），上限为 {{limit}} MiB",
      "imagePasteTooLarge": "图片过大（{{size}}），上限为 {{limit}}",
      "filePasteTooLarge": "文件过大（{{size}}），上限为 {{limit}}",
      "filePasteNoRoot": "临时传输目录不可用，无法传输文件",
      "filePasteUploaded": "文件已传输：{{path}}",
      "filePastePathCopied": "当前不可直接输入，已复制 {{count}} 个文件路径",
      "filePastePathCopyFailed": "文件已传输，但无法插入或复制路径",
      "filePasteFailed": "文件传输失败：{{message}}",
      "remoteDirectoryPasteUnsupported": "远程终端仅支持文件，已跳过 {{count}} 个文件夹",
      "pathPasteUnavailable": "有 {{count}} 个所选路径不可访问，已跳过",
      "fileDropLocalTitle": "松开以粘贴本机路径",
      "fileDropUploadTitle": "松开以传输到当前终端",
      "fileDropHint": "{{count}} 个文件 · 只插入路径，不会自动运行",
      "fileRiskTitle": "添加前请确认文件安全",
      "fileRiskDescription": "部分文件不是常见图片或文本。添加路径本身不会运行文件，但后续终端命令可能读取或执行它。仅在信任文件来源时继续。",
      "fileRiskMore": "以及另外 {{count}} 个文件",
      "fileRiskBatch": "确认后将处理共 {{count}} 个文件",
      "fileRiskConfirm": "仍要继续",
      "clearSelection": "取消选择",
  ...
  "translation": {
    ...
    "nav": {
      ...
      "forceTerminalResize": "ターミナルサイズを強制調整",
    ...
    "terminal": {
      ...
      "copiedPreview": "コピーしました:{{preview}}",
      "copyFailed": "コピーに失敗しました",
      "pasteFailed": "クリップボードを読み取れません。ブラウザの権限を確認してください",
      "pasteTooLarge": "貼り付け内容が大きすぎます（{{size}} MiB）。上限は {{limit}} MiB です",
      "imagePasteTooLarge": "画像が大きすぎます（{{size}}）。上限は {{limit}} です",
      "filePasteTooLarge": "ファイルが大きすぎます（{{size}}）。上限は {{limit}} です",
      "filePasteNoRoot": "一時転送ディレクトリが利用できないため、ファイルを転送できません",
      "filePasteUploaded": "ファイルを転送しました：{{path}}",
      "filePastePathCopied": "直接入力できないため、{{count}} 件のファイルパスをコピーしました",
      "filePastePathCopyFailed": "ファイルは転送されましたが、パスを挿入またはコピーできませんでした",
      "filePasteFailed": "ファイル転送に失敗しました：{{message}}",
      "remoteDirectoryPasteUnsupported": "リモートターミナルはファイルのみ対応しています。{{count}} 件のフォルダーをスキップしました",
      "pathPasteUnavailable": "{{count}} 件の選択したパスにアクセスできないためスキップしました",
      "fileDropLocalTitle": "放してローカルパスを貼り付け",
      "fileDropUploadTitle": "放してこのターミナルへ転送",
      "fileDropHint": "{{count}} 件のファイル · パスのみを挿入し、自動実行しません",
      "fileRiskTitle": "追加する前にファイルを確認してください",
      "fileRiskDescription": "一般的な画像またはテキストではないファイルが含まれています。パスの追加だけでは実行されませんが、後のターミナルコマンドが読み取りまたは実行する場合があります。信頼できる場合のみ続行してください。",
      "fileRiskMore": "ほか {{count}} 件",
      "fileRiskBatch": "確認後、合計 {{count}} 件のファイルを処理します",
      "fileRiskConfirm": "続行",
