...
export const I18N_RESOURCES = {
  ...
  "translation": {
    ...
    "terminal": {
      ...
      "imagePasteNoRoot": "The current pane directory is not in a writable file root",
      "imagePasteUploaded": "Image uploaded and path inserted: {{path}}",
      "imagePastePathCopied": "Image uploaded; direct input is unavailable, so the path was copied: {{path}}",
      "imagePasteFailed": "Image paste failed: {{message}}",
  ...
  "translation": {
    ...
    "terminal": {
      ...
      "imagePasteNoRoot": "当前 pane 目录未配置可写文件根，无法上传图片",
      "imagePasteUploaded": "图片已上传并插入路径：{{path}}",
      "imagePastePathCopied": "图片已上传；当前不可直接输入，路径已复制：{{path}}",
  ...
  "translation": {
    ...
    "terminal": {
      ...
      "imagePasteNoRoot": "現在のペインのディレクトリは書き込み可能なファイルルートに含まれていません",
      "imagePasteUploaded": "画像をアップロードしてパスを挿入しました：{{path}}",
      "imagePastePathCopied": "画像をアップロードしました。直接入力できないためパスをコピーしました：{{path}}",
      "imagePasteFailed": "画像の貼り付けに失敗しました：{{message}}",
